...
public class PaintJob {
    public static void main(String[] args) {
        ...
        System.out.println("enter the AreaPer bucket");
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket) {
        if (isValid(width) && isValid(height) && isValid(areaPerBucket)) {
            double totalArea = areaCalulate(width, height);
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
        if (isValid(width) && isValid(height) && isValid(areaPerBucket) && isValid(extraBuckets)) {
            double totalArea = areaCalulate(width, height);
            double currentPaintArea = areaPerBucket * extraBuckets;
            double balanceArea = totalArea - currentPaintArea;
            if (balanceArea <= 0) {
                return 0;
            } else {
                return (int) Math.ceil(balanceArea / areaPerBucket);
            }
        }
        return -1;
    }

    public static double areaCalulate(double width, double height) {
        return width * height;
    }
